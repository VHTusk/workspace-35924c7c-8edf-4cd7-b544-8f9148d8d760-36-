          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <Dialog open={showCreateModal} onOpenChange={setShowCreateModal}>
              <DialogTrigger asChild>
                <Button className={cn(isCornhole ? "bg-green-600 hover:bg-green-700" : "bg-teal-600 hover:bg-teal-700")}>
                    ...
                    {selectedPartner ? (
                      <div className="flex items-center justify-between p-3 rounded-lg bg-muted border">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-foreground">{selectedPartner.firstName} {selectedPartner.lastName}</p>
                      ...
                      <>
                        ...
                        {searchResults.length > 0 && (
                          <div className="max-h-60 overflow-y-auto border rounded-lg divide-y divide-border bg-card">
            ...
            <Card className="bg-card border-border shadow-sm mb-6">
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-lg text-foreground">
                  ...
                  Team Invitations
                  <Badge className="bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">{receivedInvitations.length} pending</Badge>
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {receivedInvitations.map((invitation) => (
                    <div key={invitation.id} className="flex items-center justify-between p-4 rounded-lg bg-muted border border-border">
                ...
                <Card key={team.id} className="bg-card border-border shadow-sm">
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {team.format || 'Doubles'} • Created {new Date(team.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-2xl font-bold text-foreground">{Math.round(team.teamElo)}</p>
                    ...
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      {/* Pending Invitation */}
                      {team.pendingInvitations.length > 0 && team.pendingInvitations.map((inv) => (
                        <div key={inv.id} className="flex items-center gap-2 p-2 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                          <Avatar className="h-8 w-8">
                            <AvatarFallback className="bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-amber-600 dark:text-amber-400">Invitation Pending</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-4 pt-4 border-t border-border">
                      ...
                      <div className="text-center">
                        ...
                        <p className="font-bold text-foreground">{team.tournamentCount}</p>
