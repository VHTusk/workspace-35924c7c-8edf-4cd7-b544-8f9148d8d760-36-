...
export default async function OfficeDistrictAdminPage() {
  const access = await getOfficeAccess(await cookies());
  if (!access) {
    redirect("/office/login");
  }
  if (!canAccessOfficeRoute(access.primaryAssignment.adminRole, AdminRole.DISTRICT_ADMIN)) {
  ...
  return (
    ...
    <section className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-semibold">District Admin</h1>
        <p className="mt-2 text-muted-foreground">Focused office access for the assigned district and sport context.</p>
      </div>
      <div className="grid gap-4 md:grid-cols-3">
        <OfficeInfoCard title="Primary scope" value={getOfficeScopeSummary(access.primaryAssignment)} />
        <OfficeInfoCard title="District" value={access.primaryAssignment.districtName || "Not assigned"} />
