...
export default async function OfficeStateAdminPage() {
  const access = await getOfficeAccess(await cookies());
  if (!access) {
    redirect("/office/login");
  }
  if (!canAccessOfficeRoute(access.primaryAssignment.adminRole, AdminRole.STATE_ADMIN)) {
  ...
  return (
    ...
    <section className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-semibold">State Admin</h1>
        <p className="mt-2 text-muted-foreground">Operate within your assigned state while keeping sport scope and governance visible.</p>
      </div>
      <div className="grid gap-4 md:grid-cols-3">
        <OfficeInfoCard title="Primary scope" value={getOfficeScopeSummary(access.primaryAssignment)} />
        <OfficeInfoCard title="State code" value={access.primaryAssignment.stateCode || "Not assigned"} />
