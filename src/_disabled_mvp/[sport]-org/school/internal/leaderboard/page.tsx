  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const leaderboardResponse = await fetch(
        `/api/org/school/leaderboard/intra?sport=${sport.toUpperCase()}`,
        { credentials: "include" }
      );
    ...
    <div className="space-y-6">
        ...
        <div className="mb-6">
          ...
          <div>
            ...
            <p className="text-gray-500">Student rankings based on internal school tournament performance</p>
          </div>
        </div>
          ...
          <>
                ...
                <div className="pt-8">
                  <Card className={cn("text-center border-2", getRankBadgeClass(2))}>
                    <CardContent className="p-4">
                      ...
                      <p className="text-xs text-gray-500">{leaderboard[1]?.className}</p>
                      {leaderboard[1]?.houseName && (
                        <div className="flex items-center justify-center gap-1 mt-1">
                          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: leaderboard[1].houseColor }} />
                          <span className="text-xs text-gray-500">{leaderboard[1].houseName}</span>
                        </div>
                      )}
                ...
                <div>
                  <Card className={cn("text-center border-2 border-yellow-400", getRankBadgeClass(1))}>
                    <CardContent className="p-4">
                      ...
                      <p className="text-xs text-gray-500">{leaderboard[0]?.className}</p>
                      {leaderboard[0]?.houseName && (
                        <div className="flex items-center justify-center gap-1 mt-1">
                          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: leaderboard[0].houseColor }} />
                          <span className="text-xs text-gray-500">{leaderboard[0].houseName}</span>
                        </div>
                      )}
                ...
                <div className="pt-12">
                  <Card className={cn("text-center border-2", getRankBadgeClass(3))}>
                    <CardContent className="p-4">
                      ...
                      <p className="text-xs text-gray-500">{leaderboard[2]?.className}</p>
                      {leaderboard[2]?.houseName && (
                        <div className="flex items-center justify-center gap-1 mt-1">
                          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: leaderboard[2].houseColor }} />
                          <span className="text-xs text-gray-500">{leaderboard[2].houseName}</span>
                        </div>
                      )}
            ...
            <Card className="bg-white border-gray-100 shadow-sm">
              <CardHeader>
                ...
                <CardDescription>All student rankings from internal school tournaments</CardDescription>
              </CardHeader>
              <CardContent>
                {leaderboard.length === 0 ? (
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-sm">Student rankings will appear after tournaments are completed</p>
                  </div>
                ) : (
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 text-xs text-gray-500">
                              ...
                              {entry.houseName && (
                                <span className="flex items-center gap-1">
                                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: entry.houseColor }} />
                                  {entry.houseName}
                                </span>
                              )}
                            </div>
