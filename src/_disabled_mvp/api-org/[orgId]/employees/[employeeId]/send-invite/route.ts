...
export async function POST(
  ...
  try {
    ...
    // Check if invite already sent and not expired
    if (
      employee.linkStatus === "INVITED" &&
      employee.inviteTokenExpires &&
      new Date() < employee.inviteTokenExpires
    ) {
      return NextResponse.json(
        { error: "Invite already sent. Use resend to send a new invite." },
        { status: 400 }
      );
    }
