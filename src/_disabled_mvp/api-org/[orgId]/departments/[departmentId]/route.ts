  ...
  try {
    ...
    const updated = await db.corporateDepartment.update({
      ...
      data: {
        ...
        code: code !== undefined ? (code ? code.toUpperCase() : null) : department.code,
    ...
    // Calculate current stats for response
    const employeeCount = await db.employee.count({
      where: { departmentId, isActive: true },
    });

    const activePlayerCount = await db.employee.count({
      where: {
        departmentId,
        isActive: true,
        isVerified: true,
    ...
    return NextResponse.json({
      ...
      department: {
        ...
        tournamentParticipations: updated.tournamentParticipations,
...
export async function DELETE(
  ...
  try {
    ...
    // Check if org still has active departments
    const remainingDepartments = await db.corporateDepartment.count({
