  ...
  try {
    ...
    return NextResponse.json({
      roster: roster.map((entry) => ({
        ...
        tier: entry.user.playerRating
          ? getEloTier(entry.user.hiddenElo, entry.user.playerRating.matchesPlayed)
          : 'UNRANKED',
        matchesPlayed: entry.user.playerRating?.matchesPlayed || 0,
        wins: entry.user.playerRating?.wins || 0,
      ...
      pendingRequests: pendingRequests
        ...
        .map((r) => ({
          ...
          tier: r.player.playerRating
            ? getEloTier(r.player.hiddenElo, r.player.playerRating.matchesPlayed)
            : 'UNRANKED',
