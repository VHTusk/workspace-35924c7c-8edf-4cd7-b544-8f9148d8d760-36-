...
import type { AuthCode, AuthField, AuthFieldErrors } from '@/lib/auth-contract';
import {
  isValidEmailAddress,
  isValidPhoneNumber,
  normalizeEmail,
...
import { OTP_TTL, otpStore, usedOtpStore, OTP_USED_PREFIX } from '@/app/api/auth/send-otp/route';
...
export async function verifyOtpPayload(payload: {
  email?: string;
  phone?: string;
  otp?: string;
  ...
  if (!normalizedEmail && !normalizedPhone) {
    return {
      success: false,
      code: AUTH_CODES.REQUIRED_FIELD_MISSING,
  ...
  if (email && (!normalizedEmail || !isValidEmailAddress(normalizedEmail))) {
    return {
      success: false,
      code: AUTH_CODES.INVALID_EMAIL_FORMAT,
  ...
  if (phone && (!normalizedPhone || !isValidPhoneNumber(normalizedPhone))) {
    return {
      success: false,
      code: AUTH_CODES.INVALID_PHONE_FORMAT,
  ...
  if (!attemptCheck.allowed) {
    return {
      success: false,
      code: AUTH_CODES.TOO_MANY_ATTEMPTS,
  ...
  if (!storedData) {
    if (usedOtp === otp) {
      return {
        success: false,
        code: AUTH_CODES.OTP_ALREADY_USED,
  ...
  await deleteOtp(identifier);
  await markOtpUsed(identifier, String(otp));
